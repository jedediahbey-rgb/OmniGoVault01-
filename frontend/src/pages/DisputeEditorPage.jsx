...
export default function DisputeEditorPage({ user }) {
  ...
  const handleAmendV2 = async (amendData) => {
    ...
    try {
      const res = await axios.post(`${API}/governance/v2/records/${disputeId}/amend`, {
  ...
  const fetchRevisions = async () => {
    try {
      const res = await axios.get(`${API}/governance/v2/records/${disputeId}/revisions`);
      const data = res.data;
  ...
  return (
    ...
    >
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex gap-2">
                ...
                <div className="flex-1 min-w-0">
                  {/* Badges row - lifecycle status, operational status, priority */}
      ...
      {/* Finalize Confirmation Modal (Global Component) */}
      <FinalizeConfirmationModal
        open={showFinalizeConfirm}
        onOpenChange={setShowFinalizeConfirm}
        onConfirm={handleFinalize}
        recordTitle={dispute?.title || 'Untitled Dispute'}
        moduleType="dispute"
        rmId={dispute?.rm_id}
