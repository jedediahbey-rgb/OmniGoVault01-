...
export default function CyberHomePage() {
  ...
  return (
    <div className="min-h-screen bg-[#05080F] text-white overflow-x-hidden">
            ...
            <div className="flex items-center gap-2" ref={userMenuRef}>
              {isLoggedIn && userData && (
                <>
                  ...
                  {/* User Avatar with StyledPortrait */}
                  <div className="relative">
                    <StyledPortrait
                      src={userData.picture}
                      alt={userData.name || 'User'}
                      fallbackText={userData.name || userData.email || 'U'}
                      styleId={userData.portrait_style || 'standard'}
                      size="sm"
                      showAccent={true}
                    ...
                    <AnimatePresence>
                      {userMenuOpen && (
                        <motion.div
                          ...
                          className="absolute right-0 top-10 w-52 bg-[#0B1221] border border-white/10 rounded-xl shadow-xl overflow-hidden z-50"
                        >
                          {/* User Info */}
                          <div className="px-4 py-3 border-b border-white/10">
                            <p className="text-sm text-white font-medium truncate">{userData.display_name || userData.name}</p>
                            <p className="text-xs text-white/40 truncate">{userData.email}</p>
                          </div>
                          
                        {/* Menu Items */}
                        <div className="py-1">
                          {/* Customize Portrait - Premium Feature */}
                          <button
                            type="button"
                            onClick={(e) => {
                              e.preventDefault();
                            ...
                            className="flex items-center gap-3 w-full px-4 py-2.5 text-sm text-vault-gold hover:text-vault-gold/80 hover:bg-vault-gold/10 transition-colors text-left"
                          >
      ...
      {/* ===== HERO SECTION ===== */}
      <section className="relative min-h-[calc(100vh-64px)] flex flex-col justify-center pb-16 sm:pb-20 lg:min-h-[62vh] lg:pb-8 lg:pt-16">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
          <div className="max-w-2xl mx-auto text-center">
            ...
            >
              ...
              <motion.h2
                variants={fadeInUp}
                className="text-lg sm:text-xl lg:text-lg text-slate-300 font-light"
              >
                A{' '}
                <span 
              ...
              <motion.div variants={fadeInUp} className="mt-4 sm:mt-4 lg:mt-4 flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4">
                ...
                {/* Create Account button - only shown for logged-out users */}
                {!isLoggedIn && (
                  <Button 
                    size="default" 
                    variant="outline"
